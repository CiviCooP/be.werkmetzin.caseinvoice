        ...
        {capture assign=sectionHeaderTemplate}
            ...
            {foreach from=$sections item=section key=column name=sections}
                ...
                <tr class="crm-report-sectionHeader crm-report-sectionHeader-{$h}{if $section.pageBreak} page-break{/if}"><th colspan="{$columnCount}">
                        <h{$h}><a href="{$l}crmURL p='civicrm/contact/view/case' q="reset=1&action=view&id=`$parent_case_id`&cid=`$parent_contact_id`"{$r}">{$l}$printValue|default:"<em>none</em>"{$r}
                                ({$l}sectionTotal key=$row.{$column} depth={$smarty.foreach.sections.index}{$r})</a>
                        </h{$h}>
                    </th></tr>
